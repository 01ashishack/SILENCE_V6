...
print("Starting Supabase DB connection attempts using raw IPv6...", flush=True)

success = False
for pwd in passwords:
    try:
        print(f"Trying password '{pwd}' on raw IPv6 port {port}...", flush=True)
...
if not success:
    print("Failed to connect using raw IPv6 address.", flush=True)
    sys.exit(1)
