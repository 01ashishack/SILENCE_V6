...
required_cols = {
    "users": ["id", "full_name", "photo_url", "subscription_plan", "subscription_status", "subscription_expiry", "role"],
    "libraries": ["id", "owner_id", "name", "address_city", "address_state", "photos", "status", "verified", "verified_at"],
    "memberships": ["id", "member_id", "library_id", "status", "end_date"],
    "seats": ["id", "library_id", "status"]
}

for table, cols in required_cols.items():
    print(f"\nChecking table '{table}':")
    for col in cols:
        # Query with select=column_name and limit=1
